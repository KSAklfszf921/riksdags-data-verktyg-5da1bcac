...
const MemberLanguageAnalysis = ({ memberId, memberName }: MemberLanguageAnalysisProps) => {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      console.log(`Loading language analysis data for ${memberName} (${memberId})`);
      ...
      console.log('Language analysis data loaded:', { 
        summaryData, 
        analysesData: analysesData.length, 
        validationData 
      });
    } catch (error) {
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center space-x-4 text-sm text-gray-600 mt-1">
                        ...
                        <span className="capitalize">
                          {analysis.document_type === 'speech' ? 'Anförande' : 'Dokument'}
                        </span>
                      </div>
                    </div>
      ...
      {!summary && !loading && (
        <Card>
          <CardContent className="py-8 text-center">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">Ingen språkanalys ännu</h3>
            <p className="text-gray-600 mb-4">
              Starta en analys för att se språklig kvalitet och komplexitet för {memberName}.
            </p>
            {validation && validation.total_analyzable > 0 && (
